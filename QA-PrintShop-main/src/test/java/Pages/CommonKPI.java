...
public class CommonKPI {
    ...
    public ArrayList<KpiClass> getKpi() {
        ...
        for (WebElement element : kpi) {
            ...
            List<WebElement> v4 = element.findElements(By.xpath(".//descendant::small/span[text()]")); // added this one in to catch the Printed KPI
            if (v1.size() > 0) {
                values = v1;
                title = "";
            } else if (v2.size() > 0) {
            ...
            for (WebElement ele2 : values) {
                ...
                // For the Printed KPI, which has two values.
                List<WebElement> subtitle = ele2.findElements(By.xpath(".//parent::small"));
                if (subtitle.size() > 0) {
                    StringBuilder subtitleText = new StringBuilder();
                    String letterExtraction = subtitle.get(0).getText();
                    for (int i = 0; i < letterExtraction.length(); i++) {
                        if (!Character.isDigit(letterExtraction.charAt(i))) {
                            subtitleText.append(letterExtraction.charAt(i));
                        }
                    }
                    cTitle += " " + subtitleText.toString().trim();
                }
    ...
    public boolean KPIExists(String kpiName) {
        for(KpiClass KPI:getKpi()){
            if(KPI.getKpiTitle().equals(kpiName)){
                return true;
            }
        }
        return false;
