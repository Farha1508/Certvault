...
public class LoginSteps extends BaseUtil {
    ...
    public void iAmTakenToThePage(String pageName) {
        Assert.assertEquals(login.pageTitle(), pageName);
    }

    @When("I click the {string} button on the Reset Password page")
    public void iClickTheButtonOnTheResetPasswordPage(String buttonName) {
        // This method exists because the "buttons" on this page are not real buttons (or links); they are "spans"
        System.out.println("Clicking on the "+buttonName+" button on the Reset Password page.");
        login.clickPassResetBtn(buttonName);
    }

    @Then("The following roles exist")
    public void theFollowingRolesExist(List<String> userList) {
        pageLoaded();
        for (String item : userList) {
            Assert.assertTrue(driver.findElement(By.xpath("//h4[normalize-space(text()) = '" + item + "']")).isDisplayed());
        }
    }
