...
public class CommonKPI {
    ...
    public void clickKpi(String kpi) {
        if(kpi.equalsIgnoreCase("past due")){
            // added this check because the "Past Due" flag was catching the "Pending Responses Past Due" KPI link
            driver.findElement(By.id("kpi6")).click();
            return;
        }

        List<WebElement> kpiLinks= driver.findElements(By.xpath("//a[contains(translate(@id, 'KPI', 'kpi'), 'kpi')]/descendant::text()[normalize-space()='" + kpi + "']/ancestor::a"));
        if(kpiLinks.isEmpty()){
            // This exception was added to catch the "Due in 30 Minutes" KPI, which has a non-breaking space in its text
            kpiLinks= driver.findElements(By.xpath("//a[contains(translate(@id, 'KPI', 'kpi'), 'kpi')]/descendant::text()[contains(.,'" + kpi + "')]/ancestor::a"));
            if(kpiLinks.isEmpty()){
                Assert.fail("Could not find " + kpi + " KPI.");
            }
        }
        js.executeScript("arguments[0].click()", kpiLinks.get(0));
        BaseUtil.pageLoaded();
    }
