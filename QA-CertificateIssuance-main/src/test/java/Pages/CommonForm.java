...
public class CommonForm {
    ...
    public WebElement commonField(String field) {
        ...
        // This is to catch the Comments fields
        fields.addAll(driver.findElements(By.xpath(
    ...
    public String commonFieldRead(String field) {
        ...
        try {
            if (selectedField.getAttribute("value") == null) { //The tag name is for the comments field.
    ...
    public WebElement commonDropDown(String dropDown) {
        ...
        // This will catch "SLA Miss Reason," which has a space in it.
        lists.addAll(driver.findElements(By.xpath(
                "//label[contains(.,'" + dropDown + "')]/following-sibling::select")));
    ...
    public WebElement commonDate(String datePicker) {
        ...
        List<WebElement> pickers = driver.findElements(By.xpath(
                "//label[normalize-space() = '" + datePicker + "']/following-sibling::div//span[@class='glyphicon glyphicon-calendar']"));
    ...
    public WebElement commonCheckBox(String checkBox) {
        ...
        checkBoxes.addAll(driver.findElements(By.xpath(
                "//label[normalize-space()='" + checkBox + "']/following-sibling::input[@type='checkbox']"))); //used in Second QA tab
...
//
    ...
    public WebElement commonConfirmModal() {
        wait.until(ExpectedConditions.presenceOfElementLocated(By.className("jconfirm")));
        WebElement selectedModal = null;
        List<WebElement> modals = driver.findElements(By.className(
                "jconfirm-content"));

        if (modals.size() > 0) {
            long startTime = System.currentTimeMillis();
            while (selectedModal == null && (System.currentTimeMillis() - startTime) < 5000) {
                try {
                    Thread.sleep(500);
                } catch (InterruptedException ignored) {
                }
...
//
    public boolean elementFound(String targetElement){
        ...
        switch (targetElement){
            case "Add Work Order":
                element = workOrderModal;
                break;
            case "Time Tracking":
                element = timerModal;
                break;
            case "Time Record Detail":
                element = timeRecordMordal;
                break;
            case "Email":
                element = emailModal;
                break;
...
//
    ...
    @FindBy(how = How.ID, using = "cert_WoNumber1")
    private WebElement workOrderNumber;

    @FindBy(how = How.ID, using = "addWorkOrder")
    private WebElement workOrderModal;

    @FindBy(how = How.ID, using = "startTimerID")
    public WebElement timerModal;

    @FindBy(how = How.ID, using = "startTimerID")
    public WebElement stopButton;

    @FindBy(how = How.ID, using = "send_email")
    public WebElement emailModal;

    @FindBy(how = How.ID, using = "Edit_TimeRecord")
    public WebElement timeRecordMordal;
}
